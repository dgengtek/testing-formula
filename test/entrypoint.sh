...
PILLARDIR=${PILLARDIR:-./pillar}
BUILDDIR=${BUILDDIR:-./build}
VENV_DIR=${VENV_DIR:-${BUILDDIR}/virtualenv}
GOSS_DOWNLOAD_URL=$(awk '/goss_download_url/ {print $2}' "$METADATA")
...
install_goss() {
  sudo curl -L "$GOSS_DOWNLOAD_URL" -o /usr/local/bin/goss
...
test_env_cleanup() {
    info "Cleaning up ${BUILDDIR}"
    [ -d ${BUILDDIR} ] && rm -rf ${BUILDDIR} || exit 0
}

test_env_prepare() {
    if [[ -f ${BUILDDIR}/.prepare_done ]]; then
      info "${BUILDDIR}/.prepare_done exist, not rebuilding BUILDDIR"
      return
    fi
